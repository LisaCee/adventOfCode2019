...
function alarm(input) {
  ...
  for (let i = 0; i < input.length; i+= 4) {
    let dest = input[i + 3]
    let first = input[input[i + 1] ]
    let second = input[input[i + 2]]
    if (input[i] == 1){
      temp = first + second
      input[dest] = temp
    } else if (input[i] == 2) {
